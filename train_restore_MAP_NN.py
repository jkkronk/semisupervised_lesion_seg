...
if __name__ == "__main__":
    ...
    for subj in subj_list: # Iterate every subject
        ...
        for ep in range(epochs):
            for batch_idx, (scan, seg, mask) in enumerate(subj_loader):
                ...
                writer.add_image('Batch of Restored', normalize_tensor(np.expand_dims(restored_batch_resized, axis=1)[:16]), batch_idx,dataformats='NCHW')
                writer.add_image('Batch of Diff Restored Scan', normalize_tensor(np.expand_dims(error_batch, axis=1)[:16]),batch_idx, dataformats='NCHW')
    ...
    path = '/scratch_net/biwidl214/jonatank/logs/restore/' + model_name + '.pth'
    torch.save(net, path)
