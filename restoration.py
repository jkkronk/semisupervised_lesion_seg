...
def train_run_map_NN(input_img, dec_mu, net, vae_model, riter, step_size, device, writer, input_seg, mask,
                     train=True,log=True, K_actf=1):
    # Init params
    input_img = input_img.to(device)
    mask = mask.to(device)
    dec_mu = dec_mu.to(device).float()
...
def run_map_GGNN(input_img, mask, dec_mu, net, vae_model, riter, device, input_seg=None, threshold=None, writer=None, step_size=0.003, log=True):
    ...
    if log and not writer == None :
        ...
        writer.add_image('out', normalize_tensor(out[:16].unsqueeze(1)), dataformats='NCHW')
