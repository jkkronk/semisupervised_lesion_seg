...
if __name__ == "__main__":
    ...
    for epoch in range(epochs_unsupervised):
        ...
        if epoch % log_freq == 0 and not epoch == 0:
            ...
            writer_valid_noskip.add_image('Original', normalize_tensor(log_sujb[:16]), epoch, dataformats='NCHW')
            writer_valid_noskip.add_image('Reconstruted', normalize_tensor(enc_log_subj[:16]), epoch, dataformats='NCHW')
