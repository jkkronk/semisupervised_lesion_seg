...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_model + model_name + 'test')
    ...
    with torch.no_grad():
        ...
        for subj in subj_list:
            ...
            for batch_idx, (scan, seg, mask) in enumerate(subj_loader): #tqdm(enumerate(test_data_loader), total=len(test_data_loader), desc='Evaluate'):
                ...
                writer.add_image('Img', normalize_tensor(scan[:16]), dataformats='NCHW')
                writer.add_image('Pred Seg', normalize_tensor(pred_seg[:16].type(torch.int8)), dataformats='NCHW')
                writer.add_image('Seg', normalize_tensor(seg[:16]), dataformats='NCHW')
        ...
        pred_list = np.rint(pred_list)
        seg = np.array(seg_list)
        pred = np.array(pred_list)

        TP = np.sum(seg[pred == 1])
        FN = np.sum(seg[pred == 0])
        FP = np.sum(pred[seg == 0])

        print('Training Dice:', (2 * TP) / (2 * TP + FN + FP))

        save_list = np.zeros((2, len(seg_list)))
        save_list[0] = seg_list
        save_list[1] = pred_list

        f = open(log_dir + 'results/' + model_name, 'wb')
